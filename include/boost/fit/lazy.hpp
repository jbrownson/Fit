...
struct placeholder_transformer
{
    ...
    struct transformer
    {
        ...
        (detail::get_args<std::is_placeholder<T>::value>(boost::fit::forward<Ts>(xs)...));
    };
...
(
    p(boost::fit::detail::pick_transformer(boost::fit::forward<T>(x)))
);
...
struct lazy_unpack
{
    ...
    constexpr auto operator()(Ts&&... xs) const BOOST_FIT_RETURNS
    (
        f(lazy_transform(boost::fit::forward<Ts>(xs), p)...)
    );
};
...
: detail::compressed_pair<F, Pack>
{
    ...
    : base_type(boost::fit::forward<X>(x), boost::fit::forward<Y>(y))
    {}
#endif
    ...
    (
        BOOST_FIT_MANGLE_CAST(const Pack&)(BOOST_FIT_CONST_THIS->get_pack(xs...))(
            boost::fit::detail::make_lazy_unpack(
                ...
                pack_forward(boost::fit::forward<Ts>(xs)...)
            )
        )
    );
};
...
{
    return lazy_invoker<F, Pack>(boost::fit::move(f), boost::fit::move(pack));
}
...
{
    return lazy_nullary_invoker<F>(boost::fit::move(f));
}
}
...
struct lazy_adaptor : detail::callable_base<F>
{
    ...
    (
        boost::fit::detail::make_lazy_invoker(BOOST_FIT_RETURNS_C_CAST(detail::callable_base<F>&&)(BOOST_FIT_CONST_THIS->base_function(x, xs...)), 
            pack(boost::fit::move(x), boost::fit::move(xs)...))
    );
