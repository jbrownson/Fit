...
struct capture_invoke : detail::callable_base<F>, Pack
{
    ...
    constexpr capture_invoke(X&& x, Y&& y) : detail::callable_base<F>(boost::fit::forward<X>(x)), Pack(boost::fit::forward<Y>(y))
    {}
    template<class... Ts>
...
struct capture_pack : Pack
{
    ...
    (
        capture_invoke<F, Pack>(boost::fit::move(f), 
...
struct make_capture_pack_f
{
    ...
    {
        return capture_pack<Pack>(boost::fit::move(p));
    }
};
...
struct capture_f
{
    ...
    (
        BOOST_FIT_RETURNS_CONSTRUCT(make_capture_pack_f)()(BOOST_FIT_RETURNS_CONSTRUCT(F)()(boost::fit::forward<Ts>(xs)...))
    );
};
}
