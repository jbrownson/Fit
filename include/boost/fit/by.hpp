...
struct project_eval
{
    ...
    constexpr project_eval(X&& xp, const P& pp) : x(boost::fit::forward<X>(xp)), p(pp)
    {}

    constexpr auto operator()() const BOOST_FIT_RETURNS
    (p(boost::fit::forward<T>(x)));
};

template<class T, class Projection>
constexpr project_eval<T, Projection> make_project_eval(T&& x, const Projection& p)
{
    return project_eval<T, Projection>(boost::fit::forward<T>(x), p);
}
...
struct project_void_eval
{
    ...
    template<class X, class P>
    constexpr project_void_eval(X&& xp, const P& pp) : x(boost::fit::forward<X>(xp)), p(pp)
    {}
    ...
    {
        return p(boost::fit::forward<T>(x)), void_();
    }
};
...
{
    return project_void_eval<T, Projection>(boost::fit::forward<T>(x), p);
}
...
{
    return apply_eval(f, make_project_eval(boost::fit::forward<Ts>(xs), p)...);
}
...
{
    return apply_eval(always(), make_project_void_eval(boost::fit::forward<Ts>(xs), p)...);
}
...
struct by_adaptor : detail::callable_base<Projection>, detail::callable_base<F>
{
    ...
    : detail::callable_base<Projection>(boost::fit::forward<P>(p)), detail::callable_base<F>(boost::fit::forward<G>(f))
    {}
...
struct by_adaptor<Projection, void> : detail::callable_base<Projection>
{
    ...
    : detail::callable_base<Projection>(boost::fit::forward<P>(p))
    {}
    ...
    {
#if BOOST_FIT_NO_ORDERD_BRACE_INIT
        return detail::by_void_eval(this->base_projection(xs...), boost::fit::forward<Ts>(xs)...);
#else
#if BOOST_FIT_NO_CONSTEXPR_VOID
        return
#endif
        detail::swallow{
            (this->base_projection(xs...)(boost::fit::forward<Ts>(xs)), 0)...
        };
#endif
    }
};
