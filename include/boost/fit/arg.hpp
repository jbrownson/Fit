...
struct perfect_ref
{
    typedef T&& type;
    T&& value;
    constexpr perfect_ref(T&& x) : value(boost::fit::forward<T>(x))
    {}
};
...
struct args_at
{
    ...
    BOOST_FIT_RETURNS(boost::fit::forward<typename T::type>(x.value));
};
...
(
    make_args_at(typename gens<N>::type())(nullptr, make_perfect_ref(boost::fit::forward<Ts>(xs))...)
);
...
struct make_args_f
{
    ...
    (
        get_args<N>(boost::fit::forward<Ts>(xs)...)
    );
};
...
(
    detail::get_args<N>(boost::fit::forward<Ts>(xs)...)
);
