...
struct compose_kernel : detail::compressed_pair<F1, F2>
{
    ...
    (
        BOOST_FIT_MANGLE_CAST(const F1&)(BOOST_FIT_CONST_THIS->first(xs...))(
            BOOST_FIT_MANGLE_CAST(const F2&)(BOOST_FIT_CONST_THIS->second(xs...))(boost::fit::forward<Ts>(xs)...)
        )
    );
};
}
...
struct compose_adaptor : detail::compose_kernel<detail::callable_base<F>, BOOST_FIT_JOIN(compose_adaptor, detail::callable_base<Fs>...)>
{
    ...
    template<class X, class... Xs, BOOST_FIT_ENABLE_IF_CONVERTIBLE(X, detail::callable_base<F>), BOOST_FIT_ENABLE_IF_CONSTRUCTIBLE(tail, Xs...)>
    constexpr compose_adaptor(X&& f1, Xs&& ... fs) 
    : base_type(boost::fit::forward<X>(f1), tail(boost::fit::forward<Xs>(fs)...))
    {}
};
